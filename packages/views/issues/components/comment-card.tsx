...
function useEditAttachmentState(
  ...
  const startEdit = (fullContent = entry.content ?? "") => {
    cancelledRef.current = false;
    const draft = getDraft(draftKey) ?? fullContent;
    setInitialValue(draft);
    setInitialContentBase(fullContent);
    setContent(draft);
...
function CommentRow({
  ...
  const { data: fullComment, isFetching: loadingFullContent, isError: loadError, refetch: refetchFullContent } = useQuery({
    ...commentDetailOptions(entry.id, entry.revision ?? entry.updated_at),
    enabled: shouldFetchFull,
  });
  const expandedContent = isTruncated ? fullComment?.content : undefined;
  const effectiveContent = expandedContent ?? entry.content ?? "";
  const isExpanded = fullComment?.id === entry.id;
  const edit = useEditAttachmentState(issueId, { ...entry, content: effectiveContent }, onEdit);

  const fullContentForAction = async () => {
    if (!isTruncated || isExpanded) return effectiveContent;
    setShouldFetchFull(true);
    const { data } = await refetchFullContent();
    return data?.content;
  };
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={async () => {
                const content = await fullContentForAction();
                if (content !== undefined && await copyText(content)) {
              ...
              {(canEditEntry || canDeleteEntry) && (
                <>
                  ...
                  {canEditEntry && (
                    <DropdownMenuItem onClick={async () => {
                      const content = await fullContentForAction();
                      if (content !== undefined) edit.startEdit(content);
                    }}>
        ...
        <>
          ...
          {isTruncated && !isExpanded && (
            <Button
              ...
              onClick={() => {
                setShouldFetchFull(true);
                if (loadError) void refetchFullContent();
              }}
              disabled={loadingFullContent}
              aria-busy={loadingFullContent}
            >
              {loadingFullContent ? (
          ...
          <AttachmentList attachments={entry.attachments} content={effectiveContent} className="mt-1.5 pl-12 pr-4 max-md:pl-3 max-md:pr-3" />
            ...
            data-comment-content={entry.id} className="pl-8 max-md:pl-0 text-body leading-relaxed text-foreground">
                ...
                {isTruncated && !isExpanded && (
                  <Button
                    ...
                    size="xs"
                    type="button"
                    className="pl-8 max-md:pl-0 pt-0.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
                    onClick={() => {
                      setShouldFetchFull(true);
                      if (loadError) void refetchFullContent();
                    }}
                ...
                <AttachmentList attachments={entry.attachments} content={effectiveContent} className="mt-1.5 pl-8 max-md:pl-0" />
                {retryableAgentFailureComment(entry) && (
                  <TaskCommentRetryButton
                    issueId={issueId}
                    taskId={entry.source_task_id}
