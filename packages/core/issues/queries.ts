...
export function commentDetailOptions(commentId: string, revision?: number | string) {
  return queryOptions({
    queryKey: [...issueKeys.comment(commentId), revision],
    queryFn: async () => {
      const comment = await api.getComment(commentId);
      if (!comment.id) throw new Error("Could not read comment");
      return comment;
    },
    staleTime: 5 * 60 * 1000,
  });
