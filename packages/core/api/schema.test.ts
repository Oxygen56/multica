...
describe("ApiClient schema fallback", () => {
  ...
  describe("getComment", () => {
    it("falls back to a safe comment when the detail response is malformed", async () => {
      stubFetchJson({ content: { unexpected: true } });
      const client = new ApiClient("https://api.example.test");
      await expect(client.getComment("comment-1")).resolves.toMatchObject({ id: "", content: "" });
    });
  });
